...
any L4 edits, builds, or runtime deploy. Then Phase 2 (L4 + one post-L4 build +
validation) if implementation is in scope.

After runtime deploy, run the **stand health gate** before Jaeger or e2e pass/fail
(`recipes/stand-health-gate.md` in the skill). Do not leave validation in a state
