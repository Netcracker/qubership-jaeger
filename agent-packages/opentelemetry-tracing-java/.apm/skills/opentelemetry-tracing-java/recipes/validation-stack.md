...
service-specific values from the repo, never hard-code them.
...
- user names a deploy environment with sufficient permissions.
...
| Service deps | Whatever the target needs to start | derive from repo docs/config |
...
and Pure Java, do not re-declare `otel.*` on top of a correctly built image.
...
crash-loops) **does not** count as e2e pass.
