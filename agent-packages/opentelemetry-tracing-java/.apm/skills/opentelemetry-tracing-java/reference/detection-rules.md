...
| `OTEL_PROPAGATORS`                                                                                             | propagation                 | `tracecontext`, `b3`, `b3multi`, `jaeger` — one list; **all** its formats are injected, extract winner is last |
| `quarkus.otel.propagators`                                                                                     | propagation                 | **build-time** (`@ConfigRoot(BUILD_AND_RUN_TIME_FIXED)`) — a format change requires a rebuild; extract winner last |
| `management.tracing.propagation.produce`                                                                       | propagation (inject)        | Boot outbound formats — **all** listed are written; **absent ⇒ default `[W3C]`**, record the default |
| `management.tracing.propagation.consume`                                                                       | propagation (extract)       | Boot inbound formats; **absent ⇒ default `[W3C, B3, B3_MULTI]`**, extract winner first          |
| `management.tracing.propagation.type`                                                                          | propagation                 | a **list**, not a single value — when set it **overrides both** `produce` and `consume`, making them dead config |
