...
`reference/service-installation-discovery.md`; the steps below are the same everywhere.
...
Record every path found in `validationPlan.runtime.scenario` or the L5 brief with file
citations. If several guides exist, prefer the one matching the **same scope** the migration
targets (Helm production vs a local bootstrap flow). Reserve plan root `gaps` for blockers:
missing docs, missing credentials, unclear install scope, skipped runtime.
...
- prerequisites and secrets (DB, tokens, volumes) and the env-injection surface where `TRACING_*` reaches the container;
- test command or traffic-generation method.

Follow the documented commands with **one** mandatory deviation: the L5 image tag
is session-unique, never the documented `:latest`/`:dev` tag. Provenance rules and
what a reused tag costs: [`build-preconditions.md`](build-preconditions.md).
...
`manual`. Do not improvise around it — no agent overlays, no stock image without
the migration changes, no chaining local clones to force a build — unless the user
explicitly asks to build and supplies the credentials or steps.

## Step 3 — No install doc: integration tests

When no installation guide exists, check whether integration tests define a repeatable run path
(test modules, build profiles, scripts under `.github/scripts/` or `bootstrap/`, documented env vars).
...
For a throwaway tracing backend only, see the language `recipes/validation-stack.md` —
use it **after** the service install path is known, not as a substitute for it.
