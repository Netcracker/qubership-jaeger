---
name: troubleshoot-jaeger
description: Use when assessing or diagnosing a support ticket involving Qubership Jaeger (Jaeger, Jaeger v2) or its components (collector, query, Cassandra storage, OpenSearch/Elasticsearch storage, readiness probe, Envoy auth proxy, Helm install), including installation, configuration, and runtime failures. Read-only and advisory; no live system access.
---
...
Diagnose one reported problem with Qubership Jaeger. Read the user's description and any attached evidence, match it to
a case in the reference, and return a diagnosis the operator can act on.
...
2. **Localize.** From the text and the log signatures, name the **component** (collector, query, Cassandra storage,
   OpenSearch/Elasticsearch storage, readiness probe, Envoy auth proxy, Helm install).
