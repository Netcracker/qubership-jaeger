...
Ingress to Gateway API without downtime:

* `legacy-ingress` (or an empty value) - `Ingress` resources are created for every component with
  `ingress.install: true`.
* `gateway-api-default` - `HTTPRoute` (or `GRPCRoute`) resources are created for every component with
  `ingress.install: true`, and no `Ingress` resources are created.
